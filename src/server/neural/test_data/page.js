...
const { Page, PageView, UserSession } = require('../../models/index')
const { randomIntBetween, randomItemFrom } = require('./utils')
const { generateClientTestData, testDomainUrl } = require('./clients')
 ...
 */
const END_VIEW_COUNT = 10000  // Ten thousand views per day
...
const generateTestPages = (clientId)=> flattenedPagePaths
...
const Run = (destroy=false)=> (destroy ? deleteRandomAccounts() : Promise.resolve(true))
    .then(()=> {
        console.log('Generating Client Test Data...')
        return generateClientTestData()
    }).then(()=> {
        console.log('Generating test accounts & users')
        return generateRandomAccounts()
    }).then(()=> {
        console.log('About to generate Test Page Views.  This may take several HOURS')
